...
4. LEARNING RATE
{'=' * 20}
Final Learning Rate: {phase_results['final_lr']:.6f}
...
def train_phase(phase_config, previous_history=None):
    ...
    completion_info = (f"\n[{phase_name}] Training completed!\n"
                      ...
                      f"  ↳ Final Learning Rate: {final_lr:.6f}")
...
# ... (resume_training 和 show_training_status 函数保持不变)
...
def show_training_status():
    ...
    if completed_phases:
        print(f"\nCompleted phases (Test Loss on {TEST_SUBSET_SIZE} samples):")
