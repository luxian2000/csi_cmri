...
def train_phase(phase_config, previous_history=None):
    ...
    for batch_num in range(1, total_batches + 1):
        ...
        def closure():
            ...
            loss.backward()
            # 梯度裁剪
            torch.nn.utils.clip_grad_norm_([C_WEIGHT, C_BIAS, Q_WEIGHT], max_grad_norm)
            return loss
        ...
        # 验证阶段用于早停机制
        if batch_num % 20 == 0:  # 每20个批次验证一次
            with torch.no_grad():
                current_val_loss = loss_fn(C_WEIGHT, C_BIAS, Q_WEIGHT, val_subset).item()
            ...
            if patience_counter >= patience:
                print(f"[{phase_name}] Early stopping triggered at batch {batch_num}")
                # 恢复最佳模型状态
                C_WEIGHT.data = best_model_state['C_WEIGHT'].data.clone()
                C_BIAS.data = best_model_state['C_BIAS'].data.clone()
                Q_WEIGHT.data = best_model_state['Q_WEIGHT'].data.clone()
                break
    ...
    phase_history = {
        ...
        'val_time': val_time,
        'learning_rate': base_lr,  # 使用初始学习率记录
    ...
    phase_info = (f"[{phase_name}] Training completed: "
                 ...
                 f"Final LR: {scheduler.get_last_lr()[0]:.6f}, "
    ...
    completion_info = (f"\n[{phase_name}] Training completed!\n"
                      ...
                      f"  ↳ Final Learning Rate: {scheduler.get_last_lr()[0]:.6f}\n"
