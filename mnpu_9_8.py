...
LOCAL = "./"
FOLDER = "model_30km_mindquantum_ascend/"
...
{'=' * 20}
Quantum Device: MindQuantum with Ascend NPU backend
...
def strong_entangling_ansatz(qubits, layers=4, prefix='ansatz'):
    ...
    for i_layer in range(layers):
        ...
        for i_qubit in range(qubits):
            ansatz += RX(f'{prefix}_l{i_layer}_q{i_qubit}_rx').on(i_qubit)
            ansatz += RY(f'{prefix}_l{i_layer}_q{i_qubit}_ry').on(i_qubit)
            ansatz += RZ(f'{prefix}_l{i_layer}_q{i_qubit}_rz').on(i_qubit)
        
        # CNOT纠缠门，使用循环连接
        for i_qubit in range(qubits):
            target_qubit = (i_qubit + 1) % qubits
            ansatz += CNOT.on(target_qubit, i_qubit)  # 修复：正确指定控制和目标量子比特
    return ansatz
...
class HybridQNN(nn.Cell):
    ...
    def __init__(self):
        ...
        # 构建量子线路
        self.encoder_target = Circuit()
        for i in range(TAR_QUBITS):
            self.encoder_target += H.on(i+1)
            
        self.ansatz = strong_entangling_ansatz(ALL_QUBITS, layers=4, prefix='main')
        
        # 构建完整的电路
        self.circuit = self.encoder_target + self.ansatz
        
        # 定义哈密顿量
        pauli_string = 'Z0'
        for i in range(1, ALL_QUBITS):
            pauli_string += f' Z{i}'
        hamiltonian = Hamiltonian(QubitOperator(pauli_string, -1.0))
        self.hamiltonian = hamiltonian
        
        # 创建量子模拟器
        self.simulator = Simulator('mqvector', ALL_QUBITS)
        
        # 获取电路参数名称
        self.circuit_params_names = list(self.circuit.params_name)
    ...
    def construct(self, x):
        ...
        # 简化处理：只使用部分参数
        params = []
        for i, name in enumerate(self.circuit_params_names):
            if i < OUTPUT_DIM and 'rx' in name:
                params.append(2 * y[i % OUTPUT_DIM])
            elif i < INPUT_DIM and 'ry' in name:
                params.append(-x[i % INPUT_DIM])
            else:
                params.append(0.5)  # 默认值
                
        # 量子前向传播
        expectation = self.simulator.get_expectation(self.hamiltonian, self.circuit, params)
        return expectation.real.astype(np.float32)
...
def main():
    """主程序"""
    print("Multi-Phase Quantum-Classical Hybrid Model Training with MindQuantum + Ascend")
    print("=" * 60)
